<div data-bind="visible: loadingItem" class="loading"><?php echo trans('frontend::frontend.loading') ?></div>

<form class="edit_form" data-bind="visible: !loadingItem(), submit: saveItem">
	<h2 data-bind="text: $root[$root.primaryKey]() ? '<?php echo trans('frontend::frontend.edit') ?>' :
		'<?php echo trans('frontend::frontend.createnew') ?>'"></h2>

	<!-- ko if: $root[$root.primaryKey]() -->
		<!-- ko if: $root.itemLink() -->
			<a class="item_link" target="_blank" data-bind="attr: {href: $root.itemLink()},
							text: '<?php echo trans('frontend::frontend.viewitem', array('single' => $config->getOption('single'))) ?>'"></a>
		<!-- /ko -->
	<!-- /ko -->
		...
		<!-- ko if: $data && ( $root[$root.primaryKey]() || editable ) && visible -->
			...
			<!-- ko if: type === 'image' -->
						...
						<div class="uploader" data-bind="attr: {disabled: $root.freezeForm, id: field_name + '_uploader'}, value: $root.activeItem,
												fileupload: {field: field_name, size_limit: size_limit, uploading: uploading, image: true,
															upload_percentage: upload_percentage, upload_url: upload_url}">
																<?php echo trans('frontend::frontend.uploadimage') ?></div>
						<!-- ko if: uploading -->
							<div class="uploading"
							data-bind="text: '<?php echo trans('frontend::frontend.imageuploading') ?>' + upload_percentage() + '%'"></div>
						<!-- /ko -->
					</div>
				<!-- /ko -->
				<!-- ko if: $root[field_name]() && !$root.loadingItem() -->
					<div class="image_container">
						<!-- ko if: display_raw_value && !$root.fieldIsDirty(field_name) -->
							<img data-bind="attr: {src: $root[field_name]()}" onload="window.frontend.resizePage()" />
						<!-- /ko -->
						<!-- ko if: !display_raw_value || $root.fieldIsDirty(field_name) -->
							<img data-bind="attr: {src: file_url + '?path=' + location + $root[field_name]()}" onload="window.frontend.resizePage()" />
						<!-- /ko -->
				...
				<!-- ko if: !$root[field_name]() && !editable -->
					<div class="uneditable" data-bind="text: '<?php echo trans('frontend::frontend.no_image_uploaded') ?>'"></div>
				<!-- /ko -->
			<!-- /ko -->
			...
			<!-- ko if: type === 'file' -->
					...
					<div class="upload_container" data-bind="attr: {id: field_id}">
						<div class="uploader" data-bind="attr: {disabled: $root.freezeForm, id: field_name + '_uploader'}, value: $root.activeItem,
												fileupload: {field: field_name, size_limit: size_limit, uploading: uploading,
															upload_percentage: upload_percentage, upload_url: upload_url}">
																<?php echo trans('frontend::frontend.uploadfile') ?></div>
						<!-- ko if: uploading -->
							<div class="uploading"
							data-bind="text: '<?php echo trans('frontend::frontend.fileuploading') ?>' + upload_percentage() + '%'"></div>
						<!-- /ko -->
					</div>
				<!-- /ko -->
				...
				<!-- /ko -->
				<!-- ko if: !$root[field_name]() && !editable -->
					<div class="uneditable" data-bind="text: '<?php echo trans('frontend::frontend.no_file_uploaded') ?>'"></div>
				<!-- /ko -->
			<!-- /ko -->
