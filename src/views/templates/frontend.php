<div class="table_container">
			...
			<!-- ko if: actionPermissions.create -->
				<a class="new_item"
					data-bind="attr: {href: base_url + modelName() + '/new'},
								text: '<?php echo trans('frontend::frontend.new') ?> ' + modelSingle()"></a>
			<!-- /ko -->
		</div>
	...
	<div class="page_container">
		<div class="per_page">
			<input type="hidden" data-bind="value: rowsPerPage, select2: {minimumResultsForSearch: -1, data: {results: rowsPerPageOptions},
											allowClear: false}" />
			<span> <?php echo trans('frontend::frontend.itemsperpage') ?></span>
		</div>
		<div class="paginator">
			<input type="button" value="<?php echo trans('frontend::frontend.previous') ?>"
					data-bind="attr: {disabled: pagination.isFirst() || !pagination.last() || !initialized() }, click: function() {page('prev')}" />
			<input type="button" value="<?php echo trans('frontend::frontend.next') ?>"
	...
	<div class="loading_rows" data-bind="visible: loadingRows">
		<div><?php echo trans('frontend::frontend.loading') ?></div>
	</div>

	<div class="no_results" data-bind="visible: pagination.last() === 0">
		<div><?php echo trans('frontend::frontend.noresults') ?></div>
	</div>
</div>
