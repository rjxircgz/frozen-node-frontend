...
class FrontendController extends Controller {
	...
	public function index($modelName)
	{
		...
		$this->layout->content = view("frontend::index");
	...
	public function item($modelName, $itemId = 0)
	{
		...
		{
			$view = view("frontend::index", array(
				'itemId' => $itemId,
			));
	...
	public function save($modelName, $id = false)
	{
		...
		else
		{
			...
			app('frontend_config_factory')->updateConfigOptions();
	...
	public function customModelAction($modelName)
	{
		...
		else
		{
			...
			if (is_a($result, 'Symfony\Component\HttpFoundation\BinaryFileResponse'))
			{
				...
				$this->session->put('frontend_download_response', array('file' => $file, 'headers' => $headers));

				$response['download'] = route('frontend_file_download');
			}
	...
	public function dashboard()
	{
		...
		if (config('frontend.use_dashboard'))
		{
			//set the layout dashboard
			$this->layout->dashboard = true;

			//set the layout content
			$this->layout->content = view(config('frontend.dashboard_view'));
		...
		else
		{
			...
			$home = config('frontend.home_page');
			...
			if (!$config)
			{
				throw new \InvalidArgumentException("Frontend: " .  trans('frontend::frontend.valid_home_page'));
			}
			else if ($config->getType() === 'model')
			{
				return redirect()->route('frontend_index', array($config->getOption('name')));
			}
			else if ($config->getType() === 'settings')
			{
				return redirect()->route('frontend_settings', array($config->getOption('name')));
			}
		}
	}
	...
	{
		if ($response = $this->session->get('frontend_download_response'))
		{
			$this->session->forget('frontend_download_response');
	...
	{
		//set the layout content and title
		$this->layout->content = view("frontend::settings");
	...
	{
		$config = app('itemconfig');
		$save = $config->save($this->request, app('frontend_field_factory')->getEditFields());
		...
		else
		{
			...
			return response()->json(array(
				...
				'actions' => app('frontend_action_factory')->getActionsOptions(),
			));
		}
	}
	...
	{
		if (in_array($locale, config('frontend.locales')))
		{
			$this->session->put('frontend_locale', $locale);
		}
