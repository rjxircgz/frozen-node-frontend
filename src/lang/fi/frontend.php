<?php

return array(

	/*
	|--------------------------------------------------------------------------
	| Frontend Language Lines
	|--------------------------------------------------------------------------
	|
	| en - English
	|
	*/

	'dashboard' => 'Kojelauta',
	'edit' => 'Muokkaa',
	'filters' => 'Hakuehdot',
	'loading' => 'Ladataan...',
	'createnew' => 'Luo uusi',
	'new' => 'Uusi',
	'viewitem' => 'Näytä :single',
	'id' => 'ID',
	'uploadimage' => 'Lähetä kuva',
	'imageuploading' => 'Kuvaa lähetetään',
	'uploadfile' => 'Lähetä tiedosto',
	'fileuploading' => 'File Uploading',
	'no_image_uploaded' => 'No image uploaded',
	'no_file_uploaded' => 'No file uploaded',
	'none' => 'Ei mitään',
	'all' => 'Kaikki',
	'itemsperpage' => 'riviä per sivu',
	'noresults' => 'Ei hakutuloksia',
	'backtosite' => 'Takaisin sivustolle',
	'logout' => 'Kirjaudu ulos',

	'previous' => 'edellinen',
	'next' => 'seuraava',

	'close' => 'Sulje',
	'delete' => 'Poista',
	'save' => 'Tallenna',
	'create' => 'Luo',
	'cancel' => 'Peruuta',

	'active' => 'Pieni hetki...',
	'success' => 'Onnistui!',
	'error' => 'Virhe toiminnossa',

	'valid_columns' => "Jokaisen mallin asetuksissa täytyy olla kelvollinen 'columns'-taulu",
	'valid_title' => "Jokaisen mallin asetuksissa tulee olla kelvolliset 'title' ja 'single' -kentät",
	'valid_model' => "Jokaisen mallin asetuksissa tulee olla 'model'-kenttä täytettynä",
	'valid_edit' => "Jokaisen mallin asetuksissa tulee olla kelvollinen 'edit_fields'-taulu",
	'valid_menu' => "frontend.php -asetuksissa tulee olla kelvollinen 'menu'-asetus",
	'valid_config_path' => "frontend.php -asetustiedostossa täytyy olla kelvollinen 'model_config_path'-kenttä. Tämän hakemiston pitää myös olla olemassa ja siihen pitää olla lukuoikeudet.",
	'not_eloquent' => " ei ole Eloquent-malli",
	'storage_path_permissions' => "Anna storage-polulle kirjoitusoikeudet, jotta asetukset-sivu (settings) toimisi",
	'valid_home_page' => "frontend.php:n home_page -asetuksen arvon tulee olla kelvollinen menu-kohta.",
);