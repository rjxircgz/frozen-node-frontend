<?php

return array(

	/*
	|--------------------------------------------------------------------------
	| Frontend Language Lines
	|--------------------------------------------------------------------------
	|
	| zh-CN - 简体中文
	|
	*/

	'dashboard' => '控制面板',
	'edit' => '编辑',
	'filters' => '筛选',
	'loading' => '载入中...',
	'createnew' => '新建',
	'new' => '新建',
	'viewitem' => '查看 :single',
	'id' => 'ID',
	'uploadimage' => '上传图片',
	'imageuploading' => '图片上传中',
	'uploadfile' => '上传文件',
	'fileuploading' => '文件上传中',
	'no_image_uploaded' => 'No image uploaded',
	'no_file_uploaded' => 'No file uploaded',
	'none' => '没有',
	'all' => '全部',
	'itemsperpage' => '条目每页',
	'noresults' => '没有结果',
	'backtosite' => '返回主站',
	'logout' => '退出',

	'previous' => '上一页',
	'next' => '下一页',

	'close' => '关闭',
	'delete' => '删除',
	'save' => '保存',
	'create' => '创建',
	'cancel' => '取消',

	'active' => '稍等...',
	'success' => '成功！',
	'error' => '执行此操作时产生一个错误',

	'valid_columns' => "在每个 model 的配置中你必须提供一个有效的 'columns' 数组",
	'valid_title' => "在每个 model 的配置中你必须要提供有效的标题（ title ）和单数名字（ single ）参数",
	'valid_model' => "在每个 model 的配置中你必须要提供有效的模型（ model ）参数",
	'valid_edit' => "在每个 model 的配置中你必须提供一个有效的 'edit_fields' 数组",
	'valid_menu' => "在 frontend.php 的配置中你必须提供一个有效的 'menu' 参数",
	'valid_config_path' => "在 frontend.php 的配置中你必须提供一个有效的 'model_config_path' 参数。这个文件夹必须是存在并且可读的。",
	'not_eloquent' => " 不是 Eloquent model",
	'storage_path_permissions' => "为了使用设置页面，你必须用于 storage 路径写入的权限",
	'valid_home_page' => "在 frontend.php 的配置中你必须提供一个有效的 'home_page' 参数",
);