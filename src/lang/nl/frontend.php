<?php

return array(

	/*
	|--------------------------------------------------------------------------
	| Frontend Language Lines
	|--------------------------------------------------------------------------
	|
	| nl - Dutch
	|
	*/

	'dashboard' => 'Dashboard',
	'edit' => 'Wijzig',
	'filters' => 'Filters',
	'loading' => 'Laden...',
	'createnew' => 'Nieuwe aanmaken',
	'new' => 'Nieuwe',
	'viewitem' => 'Bekijk :single',
	'id' => 'ID',
	'uploadimage' => 'Upload Afbeelding',
	'imageuploading' => 'Afbeelding Uploading',
	'uploadfile' => 'Upload File',
	'fileuploading' => 'File Uploading',
	'no_image_uploaded' => 'No image uploaded',
	'no_file_uploaded' => 'No file uploaded',
	'none' => 'Geen',
	'all' => 'Alle',
	'itemsperpage' => 'items per pagina',
	'noresults' => 'Geen Resultaten',
	'backtosite' => 'Terug naar de Site',
	'logout' => 'Afmelden',

	'previous' => 'vorige',
	'next' => 'volgende',

	'close' => 'Sluit',
	'delete' => 'Verwijder',
	'save' => 'Bewaar',
	'create' => 'Aanmaken',
	'cancel' => 'Annuleren',

	'active' => 'Momentje...',
	'success' => 'Success!',
	'error' => 'Er is een fout opgetreden tijdens het uivoeren van deze actie',

	'valid_columns' => "Je moet een geldig 'columns' array aangeven in elk model's config",
	'valid_title' => "Je moet een geldig 'title' en 'single name' aangeven in elk model's config",
	'valid_model' => "Je moet een geldige 'model' optie aangeven in elk  model's config",
	'valid_edit' => "Je moet een geldig 'edit_fields' array aangeven in elk model's config",
	'valid_menu' => "Je moet een geldige 'menu' optie aangeven in de frontend.php config",
	'valid_config_path' => "Je moet een geldig 'model_config_path' aangeven in de frontend.php config. De directory moet ook bestaan en leesbaar zijn.",
	'not_eloquent' => " is geen Eloquent model",
	'storage_path_permissions' => "You must make your storage path writable in order to make a settings page",
	'valid_home_page' => "You must provide a valid menu item in the home_page option of your config",
);