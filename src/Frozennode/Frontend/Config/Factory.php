<?php
namespace Frozennode\Frontend\Config;

use Frozennode\Frontend\Validator;
use Illuminate\Validation\Validator as CustomValidator;
use Frozennode\Frontend\Config\Settings\Config as SettingsConfig;
use Frozennode\Frontend\Config\Model\Config as ModelConfig;
...
class Factory {
	...
	public function __construct(Validator $validator, CustomValidator $custom_validator, array $options)
	{
		...
		if ($validator->fails())
		{
			throw new \InvalidArgumentException('There are problems with your frontend.php config: ' . implode('. ', $validator->messages()->all()));
		}
	}
	 ...
	 *
	 * @return \Frozennode\Frontend\Config\ConfigInterface
	 */
