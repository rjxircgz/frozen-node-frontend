...
class Menu {
	 ...
	 * The config instance
	 *
	 * @var \Frozennode\Frontend\Config\Factory
	 */
	 ...
	 * @param \Frozennode\Frontend\Config\Factory	$config
	 */
	...
	public function getMenu($subMenu = null)
	{
		...
		if (!$subMenu)
		{
			$subMenu = $this->config->get('frontend.menu');
		}
		...
		foreach ($subMenu as $key => $item)
		{
			...
			if (is_string($item))
			{
				...
				if (is_a($config, 'Frozennode\Frontend\Config\Config') && $config->getOption('permission'))
				{
