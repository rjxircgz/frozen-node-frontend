<?php
namespace Frozennode\Frontend\DataTable;
...
class DataTable {
	 ...
	 *
	 * @var \Frozennode\Frontend\DataTable\Columns\Factory
	 */
	 ...
	 * The validator instance
	 *
	 * @var \Frozennode\Frontend\Fields\Factory
	 */
	 ...
	 * @param \Frozennode\Frontend\DataTable\Columns\Factory	$columnFactory
	...
	public function setRowsPerPage(\Illuminate\Session\Store $session, $globalPerPage, $override = null)
	{
		if ($override)
		{
			...
			$session->put('frontend_' . $this->config->getOption('name') . '_rows_per_page', $perPage);
		}

		$perPage = $session->get('frontend_' . $this->config->getOption('name') . '_rows_per_page');
