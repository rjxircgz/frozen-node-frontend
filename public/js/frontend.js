	...
	frontend.prototype = {
		...
		viewModel: {
			...
			getItem: function(id)
			{
				...
				frontendData.edit_fields = self.originalEditFields;
				self.editFields(window.frontend.prepareEditFields());
			...
			setUpNewItem: function()
			{
				...
				window.frontend.runConstraintsQueue();
			},
			...
			setData: function(data)
			{
				...
				frontendData.edit_fields = data.frontend_edit_fields;
				self.editFields(window.frontend.prepareEditFields());

				//update the actions and the action permissions
				self.actions(data.frontend_actions);
				self.actionPermissions = data.frontend_action_permissions;

				//set the original values
				self.originalData = data;

				//set the new options for relationships
				$.each(frontendData.edit_fields, function(ind, el)
				{
				...
				if (data.frontend_item_link)
				{
					self.itemLink(data.frontend_item_link);
				}
			...
			getFilters: function()
			{
				...
				$.each(window.frontend.filtersViewModel.filters, function(ind, el)
				{
					var filter = {
			...
			updateSelfRelationships: function()
			{
				...
				$.each(window.frontend.filtersViewModel.filters, function(ind, filter)
				{
					var fieldIndex = ind,
					...
					{
						window.frontend.filtersViewModel.filters[fieldIndex].loadingOptions(true);
						...
						$.ajax({
							...
							complete: function()
							{
								window.frontend.filtersViewModel.filters[fieldIndex].loadingOptions(false);
							},
							success: function(response)
							{
								//update the options
								window.frontend.filtersViewModel.listOptions[fieldName](response[fieldName]);
							}
						});
		...
		init: function()
		{
			...
			this.viewModel.model = frontendData.data_model;
			this.$container = $('#frontend_content');

			var viewModel = ko.mapping.fromJS(this.viewModel.model);

			$.extend(this.viewModel, viewModel);

			this.viewModel.rows(frontendData.rows.results);
			this.viewModel.pagination.page(frontendData.rows.page);
			this.viewModel.pagination.last(frontendData.rows.last);
			this.viewModel.pagination.total(frontendData.rows.total);
			this.viewModel.sortOptions.field(frontendData.sortOptions.field);
			this.viewModel.sortOptions.direction(frontendData.sortOptions.direction);
			this.viewModel.columns(this.prepareColumns());
			this.viewModel.modelName(frontendData.model_name);
			this.viewModel.modelTitle(frontendData.model_title);
			this.viewModel.modelSingle(frontendData.model_single);
			this.viewModel.expandWidth(frontendData.expand_width);
			this.viewModel.rowsPerPage(frontendData.rows_per_page);
			this.viewModel.primaryKey = frontendData.primary_key;
			this.viewModel.actions(frontendData.actions);
			this.viewModel.globalActions(frontendData.global_actions);
			this.viewModel.actionPermissions = frontendData.action_permissions;
			this.viewModel.languages = frontendData.languages;
			...
			this.viewModel.originalEditFields = frontendData.edit_fields;
		...
		prepareFilters: function()
		{
			...
			$.each(frontendData.filters, function(ind, filter)
			{
		...
		prepareEditFields: function()
		{
			...
			$.each(frontendData.edit_fields, function(ind, field)
			{
		...
		prepareColumns: function()
		{
			...
			$.each(frontendData.column_model, function(ind, column)
			{
		...
		initRelationships: function()
		{
			...
			//set up the filters
			$.each(frontendData.filters, function(ind, el)
			{
				if (el.relationship)
		...
		initHistory: function()
		{
			...
			//if the frontend data had an id supplied, it means this is either the edit page or the new item page
			if ('id' in frontendData)
			{
				...
				var timer = setInterval(function()
				{
					if (window.frontend)
					{
						window.frontend.viewModel.getItem(frontendData.id);
						historyData.id = frontendData.id;
		...
		/**
		 * Handles a window resize to make sure the frontend area is always
		 */
		resizePage: function()
		{
			setTimeout(function()
			{
				...
				$('#frontend_page').css({minHeight: usedHeight});

				//resize or scroll the data table
				if (window.frontend) {
					if (! window.frontend.dataTableScrollable)
						window.frontend.resizeDataTable();
					else
					window.frontend.scrollDataTable();
				}
			}, 50);
		},
