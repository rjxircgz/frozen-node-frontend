(function($){
	...
	var select2Defaults = {
			placeholder: frontendData.languages['select_options'],
			formatNoMatches: function(term)
			{
				return frontendData.languages['no_results'];
			},
			width: 'resolve',
			allowClear: true
		};
	...
	var select2RemoteHandler = function (element, valueAccessor, allBindingsAccessor, viewModel, context)
	{
		var options = valueAccessor(),
			defaults = $.extend({
				...
				allowClear: true,
				ajax: {
					url: base_url + frontendData.model_name + '/update_options',
					dataType: 'json',
					quietMillis: 100,
					type: 'POST',
					data: function(term, page)
					{
						...
						if (data.type === 'edit')
						{
							data.selectedItems = frontend.viewModel[data.field]();
						}
						else if (data.type === 'filter')
						{
							data.selectedItems = frontend.filtersViewModel.filters[parseInt(options.filterIndex)].value();
						}
						...
						if (options.constraints)
						{
							...
							{
								data.constraints[ind] = frontend.viewModel[ind]();
							});
						}
					...
					results: function(returndata, page)
					{
						...
						if (val)
						{
							...
							{
								data[this] = {id: this, text: frontend.viewModel[options.field + '_autocomplete'][this].text};
							});
						}
						...
						frontend.viewModel[options.field + '_autocomplete'] = data;
				...
				initSelection: function(element, callback) {
					...
					if (options.multiple)
					{
						...
						{
							data.push({id: this, text: frontend.viewModel[options.field + '_autocomplete'][this].text});
						});
					}
					//otherwise make the data a simple object
					else
					{
						data = {id: val, text: frontend.viewModel[options.field + '_autocomplete'][val].text};
					}
...
//			text = ' character' + (left !== 1 ? 's' : '') + ' left';
			text = (left !== 1 ? frontendData.languages['characters_left'] : frontendData.languages['character_left']);
	...
	ko.bindingHandlers.wysiwyg = {
		init: function (element, valueAccessor, allBindingsAccessor, context)
		{
			...
			else
			{
				$element.ckeditor({
					language : language,
					readOnly : !frontendData.edit_fields[context.field_name].editable
				});
